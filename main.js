...
const token = 'YOUR_TOKEN'; // Replace with your Telegram bot token
...
function sendVisitors(url, numBots, delay, chatId, userAgents, referers, proxyList) {
  ...
  // Extracting user and password from proxy if provided
  ...
  if (proxyCredentials) {
    proxyConfig.auth = {
      username: proxyCredentials.split(':')[0] || '', // Extracting username if provided
      password: proxyCredentials.split(':')[1] || '' // Extracting password if provided
    };
  }
